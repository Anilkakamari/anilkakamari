if (null == pimaxColumns || pimaxColumns == "" || pimaxColumns == "undefined") var pimaxColumns = 2;
if (null == pimaxWidgetWidth || pimaxWidgetWidth == "" || pimaxWidgetWidth == "undefined") var pimaxWidgetWidth = 640;
if (null == albumNames || albumNames == "" || albumNames == "undefined") var albumNames = [];
...
if (typeof pimaxWidgetHeight !== 'undefined') {
    if (null != pimaxWidgetHeight && pimaxWidgetHeight != "" && pimaxWidgetHeight != "undefined") $('html > head').append('<style>#pimax{overflow-y:auto;height:' + pimaxWidgetHeight + 'px;}</style>')
}
...
function byteConversion(numBytes) {
    var kilobytes;
    var megabytes;
    if (null == numBytes || numBytes == "" || numBytes == "undefined") return "? KB";
    if (numBytes >= 1024) {
...
function preparePimax() {
    if (googleUserName.indexOf("@") != -1) {
        ...
        if (googleUserName.indexOf("/") != -1) googleUserName = googleUserName.substring(0, googleUserName.indexOf("/"))
    }
    $('#pimax').empty();
    loadPimax();
    showLoader()
...
function showPimaxTabs(response) {
    ...
    $('.pimax-tab').click(function() {
        ...
        pimaxTabId = this.id;
        if (pimaxTabId == "picasa-albums") getAlbums();
        else getAlbumImages(pimaxTabId)
    });
...
function showTopImagesOfAlbums(response) {
    ...
    var pimaxTnailWidth;
    var pimaxTnailHeight = $('.pimax-album-sidebar-img').css('height:auto');
    if (null != pimaxTnailHeight && pimaxTnailHeight != "undefined") {
        ...
        $('div.pimax-album-sidebar-img').css({
            'width': pimaxTnailWidth + 'px'
        });
        $('div.pimax-album-sidebar').css({
            'width': (pimaxTnailWidth) + 'px'
        })
    } else {
        $('div.pimax-album-sidebar').css({
...
function getTopImagesOfAlbums(albumIdArray, maxTopPics) {
    for (var i = 0; i < albumIdArray.length; i++) {
        ...
        $.ajax({
            ...
            dataType: 'jsonp',
            success: function(response) {
                showTopImagesOfAlbums(response)
            },
            error: function(html) {
                alert(html)
            },
...
function showAlbums(response) {
    ...
    for (var i = 0; i < albumArray.length; i++) {
        ...
        if (null != albumType && albumType != "undefined") albumType = albumType.$t;
        ...
        if ((i - zeroAlbumCompensation) % pimaxColumns != 0) $('#pimax-img-list-div').append('<div class="pimax-img-tnail-box" style="width:' + ((100 / pimaxColumns) - 4) + '%;" id="' + albumId + '"><div class="pimax-img-tnail" style="background-image:url(\'' + picThumbnail + '\')"><div class="pimax-album-sidebar" id="pimax-album-sidebar-' + albumId + '"><span class="pimax-album-img-count"><b>' + numPics + '</b><br/>pics</span></div></div><br/></div>');
        else $('#pimax-img-list-div').append('<div class="pimax-img-tnail-box" style="width:' + ((100 / pimaxColumns) - 4) + '%; clear:both;" id="' + albumId + '"><div class="pimax-img-tnail" style="background-image:url(\'' + picThumbnail + '\')"><div class="pimax-album-sidebar" id="pimax-album-sidebar-' + albumId + '"><span class="pimax-album-img-count"><b>' + numPics + '</b><br/>pics</span></div></div><br/></div>')
    }
    ...
    $('div.pimax-img-tnail').css({
        'height': pimaxTnailHeight + 'px'
    });
...
function showAlbumImages(response) {
    ...
    for (var i = 0; i < albumArray.length; i++) {
        ...
        if (i % pimaxColumns != 0) $('#pimax-img-list-div').append('<div class="pimax-img-tnail-box" style="width:' + ((100 / pimaxColumns) - 4) + '%;" data-picSrc="' + picSrc + '" data-picHeight="' + picHeight + '" id="' + picId + '"><div class="pimax-img-tnail" style="background-image:url(\'' + picThumbnail + '\')"></div><br/></div>');
        else $('#pimax-img-list-div').append('<div class="pimax-img-tnail-box" style="width:' + ((100 / pimaxColumns) - 4) + '%; clear:both;" data-picSrc="' + picSrc + '" data-picHeight="' + picHeight + '" id="' + picId + '"><div class="pimax-img-tnail" style="background-image:url(\'' + picThumbnail + '\')"></div><br/></div>')
    }
    $('.pimax-img-tnail-box').click(function() {
